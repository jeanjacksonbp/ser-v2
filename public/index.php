...
if ($uri === '/login' && $method === 'POST') {
  ...
  if (!$u || empty($u['senha_hash']) || !password_verify($senha, $u['senha_hash'])) {
    header('Location: ' . url('login?err=Credenciais+inválidas'));
    exit;
  }
  ...
  // Força recarregamento do contexto após login
  App\Core\Http\ScopeMiddleware::reload($GLOBALS['pdo']);
  ...
  if ($actor && $actor->hasRole('SuperAdmin')) { 
    header('Location: ' . url('dashboard')); // Por enquanto todos para dashboard
    exit; 
  }
  
  if ($guard->can('dashboard.org.view')) { 
    header('Location: ' . url('dashboard')); // Por enquanto todos para dashboard
    exit; 
  }
...
if ($uri === '/' || $uri === '/dashboard') {
  require __DIR__ . '/dashboard.php'; exit;
}
if ($uri === '/superadmin') {
